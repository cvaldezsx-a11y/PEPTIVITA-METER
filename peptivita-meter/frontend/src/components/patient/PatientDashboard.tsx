...
function IMCPatientWidget({ weight, height, goalWeight, initialWeight }: {
  ...
  const getCategory = (v: number) => {
    if (v < 18.5) return { label: 'Bajo peso',   color: 'var(--cyan)',  emoji: '💙', msg: 'Estás trabajando para alcanzar tu peso óptimo. ¡Cada consulta te acerca más!' }
    if (v < 25)   return { label: 'Normal ✓',    color: 'var(--green)', emoji: '💚', msg: '¡Estás en tu rango ideal! Mantén el ritmo, tu cuerpo te lo agradece.' }
    if (v < 30)   return { label: 'Sobrepeso',   color: 'var(--amber)', emoji: '🌟', msg: 'Vas por buen camino. Cada kg menos es una victoria para tu salud.' }
    if (v < 35)   return { label: 'Obesidad I',  color: '#FF8C42',      emoji: '🔥', msg: '¡Tu dedicación está dando resultados! Sigue con el plan.' }
    if (v < 40)   return { label: 'Obesidad II', color: 'var(--red)',   emoji: '💪', msg: 'El camino es largo pero ya empezaste. ¡Cada paso cuenta!' }
  ...
  return (
    <div style={{
      ...
      borderRadius: '16px', padding: '24px', marginBottom: '24px', position: 'relative', overflow: 'hidden',
    }}>
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', background: 'var(--bg-deep)' }}>
      ...
      <main style={{ flex: 1, overflow: 'auto', padding: '32px' }}>
          ...
          <div>
            ...
            {latestNotes && (
              <div style={{ background: 'linear-gradient(135deg, rgba(0,200,255,0.08), rgba(0,229,160,0.08))', border: '1px solid var(--cyan)', borderRadius: '12px', padding: '20px', marginBottom: '24px' }}>
                <div style={{ fontSize: '12px', color: 'var(--cyan)', fontWeight: 700, marginBottom: '8px', letterSpacing: '0.5px' }}>📋 NOTAS DE TU ESPECIALISTA</div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '16px', marginBottom: '24px' }}>
              ...
              {frameSize && <KPICard icon="🦴" label="Constitución Ósea" value={frameSize} color="var(--text-secondary)" />}
            </div>
            ...
            {latestAnthro?.weight_kg && latestAnthro?.height_cm && (
              <IMCPatientWidget weight={latestAnthro.weight_kg} height={latestAnthro.height_cm} goalWeight={patient.goal_weight} initialWeight={initialWeight} />
            )}
            ...
            {activePeptides.length > 0 && (
              <div className="glass-card" style={{ padding: '20px' }}>
                <h3 style={{ fontFamily: 'DM Sans', fontSize: '16px', fontWeight: 700, margin: '0 0 16px' }}>💉 Tratamiento Activo con Péptidos</h3>
        ...
        {/* ── HISTORIAL ── */}
        {tab === 'history' && <HistoryPanel consultations={consultations} anthropoHistory={anthropoHistory} />}
      </main>
    </div>
  )
}
...
function KPICard({ icon, label, value, sub, color }: { icon: string; label: string; value: string; sub?: string; color: string }) {
  return (
    <div className="glass-card" style={{ padding: '20px' }}>
      ...
      <div style={{ fontSize: '11px', color: 'var(--text-muted)', fontWeight: 600, letterSpacing: '0.5px', marginBottom: '4px' }}>{label.toUpperCase()}</div>
      <div style={{ fontSize: '22px', fontWeight: 700, color, fontFamily: 'JetBrains Mono, monospace', marginBottom: '2px' }}>{value}</div>
...
function LabPanel({ labResults }: { labResults: any[] }) {
  ...
  return (
    <div>
      <h1 style={{ fontFamily: 'DM Sans', fontSize: '24px', fontWeight: 700, margin: '0 0 24px' }}>🧪 Exámenes de Laboratorio</h1>
      {params.length === 0 && <p style={{ color: 'var(--text-secondary)' }}>Aún no hay resultados de laboratorio registrados.</p>}
        ...
        {params.map(param => {
          ...
          return (
            <button key={param} onClick={() => setSelectedParam(selectedParam === param ? null : param)}
              style={{ background: selectedParam === param ? classBg[cls] : 'var(--bg-card)', border: `1px solid ${selectedParam === param ? classColors[cls] : 'var(--border)'}`, borderRadius: '10px', padding: '16px', cursor: 'pointer', textAlign: 'left', transition: 'all 0.15s' }}>
              <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginBottom: '4px' }}>{param}</div>
              <div style={{ fontSize: '20px', fontWeight: 700, fontFamily: 'JetBrains Mono', color: classColors[cls] }}>{latest.value_numeric ?? latest.value_text ?? '—'}</div>
              <div style={{ marginTop: '6px' }}>
                <span style={{ fontSize: '11px', fontWeight: 600, padding: '2px 8px', borderRadius: '12px', background: classBg[cls], color: classColors[cls] }}>{classLabel[cls]}</span>
              </div>
            </button>
          )
        })}
      ...
      {selectedParam && grouped[selectedParam].length > 1 && (
        <div className="glass-card" style={{ padding: '20px' }}>
          <h3 style={{ fontFamily: 'DM Sans', fontSize: '16px', fontWeight: 700, margin: '0 0 16px' }}>Evolución: {selectedParam}</h3>
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <h1 style={{ fontFamily: 'DM Sans', fontSize: '24px', fontWeight: 700, margin: 0 }}>📸 Fotos de Progreso</h1>
      ...
      {(['front','side','back'] as const).map(angle => {
        ...
        return (
          <div key={angle} style={{ marginBottom: '28px' }}>
            <h3 style={{ fontSize: '14px', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: '12px', letterSpacing: '0.5px' }}>{labels[angle].toUpperCase()}</h3>
            {compareMode && anglePhotos.length >= 2 ? (
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', maxWidth: '500px' }}>
                {[anglePhotos[anglePhotos.length - 1], anglePhotos[0]].map((ph: any, idx: number) => (
                  <div key={ph.id}>
                    <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginBottom: '6px', fontWeight: 600 }}>{idx === 0 ? '✅ ACTUAL' : '⏮ INICIAL'} · {ph.photo_date}</div>
              ...
              <div style={{ display: 'flex', gap: '12px', overflowX: 'auto', paddingBottom: '8px' }}>
                {anglePhotos.map((ph: any) => (
                  <div key={ph.id} style={{ minWidth: '150px' }}>
                    ...
                    <img src={getUrl(ph.storage_path)} alt={angle} style={{ width: '150px', height: '200px', borderRadius: '8px', objectFit: 'cover', border: '1px solid var(--border)' }} />
                  </div>
                ))}
              </div>
            )}
      ...
      {photos.length === 0 && <p style={{ color: 'var(--text-secondary)' }}>Aún no se han cargado fotos de progreso.</p>}
    </div>
  )
}

// ── Historial con comparación entre consultas ────────────────
function HistoryPanel({ consultations, anthropoHistory }: { consultations: any[]; anthropoHistory: any[] }) {
  ...
  const fields = [
    ...
    { key: 'muscle_mass_kg', label: 'Masa Muscular', unit: 'kg',  better: 'up'   },
    { key: 'waist_cm',       label: 'Cintura',       unit: 'cm',  better: 'down' },
    { key: 'hip_cm',         label: 'Cadera',        unit: 'cm',  better: 'none' },
    { key: 'bmi',            label: 'IMC',           unit: '',    better: 'down' },
    { key: 'waist_hip_ratio',label: 'ICC',           unit: '',    better: 'down' },
    { key: 'bicep_left_cm',  label: 'Bíceps Izq.',   unit: 'cm',  better: 'up'   },
    { key: 'bicep_right_cm', label: 'Bíceps Der.',   unit: 'cm',  better: 'up'   },
    { key: 'thigh_left_cm',  label: 'Muslo Izq.',    unit: 'cm',  better: 'none' },
    { key: 'bp_systolic',    label: 'Presión Sist.', unit: 'mmHg',better: 'down' },
    { key: 'heart_rate',     label: 'Frec. Cardíaca',unit: 'lpm', better: 'down' },
  ]

  function getAnthro(consultId: string) {
    return anthropoHistory.find((a: any) => a.consultation_id === consultId)
  }

  function getDiff(valA: number, valB: number, better: string) {
    const diff = valB - valA
    if (diff === 0) return { text: '= Sin cambio', color: 'var(--text-muted)' }
    if (better === 'down') return diff < 0
      ? { text: `▼ ${Math.abs(diff).toFixed(1)} ✓`, color: 'var(--green)' }
      : { text: `▲ ${diff.toFixed(1)}`, color: 'var(--red)' }
    if (better === 'up') return diff > 0
      ? { text: `▲ ${diff.toFixed(1)} ✓`, color: 'var(--green)' }
      : { text: `▼ ${Math.abs(diff).toFixed(1)}`, color: 'var(--red)' }
    return diff > 0
      ? { text: `▲ ${diff.toFixed(1)}`, color: 'var(--amber)' }
      : { text: `▼ ${Math.abs(diff).toFixed(1)}`, color: 'var(--amber)' }
  }
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <h1 style={{ fontFamily: 'DM Sans', fontSize: '24px', fontWeight: 700, margin: 0 }}>📋 Historial de Consultas</h1>
        {consultations.length >= 2 && (
          <button className="btn-ghost" onClick={() => setMode(mode === 'list' ? 'compare' : 'list')} style={{ fontSize: '13px' }}>
            {mode === 'list' ? '⚖️ Comparar Consultas' : '📋 Ver Lista'}
          </button>
        )}
      </div>
        ...
        <div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '24px' }}>
            {[{ label: 'Consulta A (Base)', val: compareA, set: setCompareA, color: 'var(--cyan)' },
              ...
              <div key={col.label}>
                <div style={{ fontSize: '12px', color: col.color, fontWeight: 700, marginBottom: '8px' }}>{col.label}</div>
                ...
                >
                  ...
                  {sorted.map((c: any, i: number) => (
                    <option key={c.id} value={c.id}>
                      Consulta #{sorted.length - i} · {c.consultation_date}
                    </option>
                  ))}
                </select>
              </div>
          ...
          {anthroA && anthroB && (
            <div className="glass-card" style={{ padding: '24px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', gap: '0', marginBottom: '8px' }}>
                <div style={{ fontSize: '12px', color: 'var(--text-muted)', fontWeight: 600, padding: '8px 0' }}>MEDICIÓN</div>
                <div style={{ fontSize: '12px', color: 'var(--cyan)', fontWeight: 600, padding: '8px', textAlign: 'center' }}>{consultA?.consultation_date}</div>
                <div style={{ fontSize: '12px', color: 'var(--green)', fontWeight: 600, padding: '8px', textAlign: 'center' }}>{consultB?.consultation_date}</div>
                <div style={{ fontSize: '12px', color: 'var(--text-muted)', fontWeight: 600, padding: '8px', textAlign: 'center' }}>CAMBIO</div>
              </div>
              {fields.map(f => {
                const vA = anthroA[f.key]
                const vB = anthroB[f.key]
                if (!vA && !vB) return null
                const diff = vA && vB ? getDiff(Number(vA), Number(vB), f.better) : null
                return (
                  <div key={f.key} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', gap: '0', borderTop: '1px solid var(--border)' }}>
                    <div style={{ padding: '10px 0', fontSize: '13px', color: 'var(--text-secondary)' }}>{f.label}</div>
                    <div style={{ padding: '10px 8px', textAlign: 'center', fontFamily: 'JetBrains Mono', fontSize: '13px', color: 'var(--cyan)' }}>
                      {vA != null ? `${Number(vA).toFixed(1)} ${f.unit}` : '—'}
                    </div>
                    <div style={{ padding: '10px 8px', textAlign: 'center', fontFamily: 'JetBrains Mono', fontSize: '13px', color: 'var(--green)' }}>
                      {vB != null ? `${Number(vB).toFixed(1)} ${f.unit}` : '—'}
                    </div>
                    <div style={{ padding: '10px 8px', textAlign: 'center', fontSize: '13px', fontWeight: 600, color: diff?.color || 'var(--text-muted)' }}>
                      {diff?.text || '—'}
                    </div>
                  </div>
                )
          ...
          {(!compareA || !compareB) && (
            <p style={{ color: 'var(--text-secondary)', textAlign: 'center', padding: '32px' }}>
              Selecciona dos consultas para comparar tus medidas
            </p>
          )}
        </div>
      )}
            ...
            return (
              <div key={c.id} className="glass-card" style={{ overflow: 'hidden' }}>
                <button onClick={() => setIsOpen(!isOpen)}
                  ...
                  <div>
                    <span style={{ fontWeight: 700, marginRight: '12px' }}>Consulta #{sorted.length - i}</span>
                    <span style={{ color: 'var(--text-secondary)', fontSize: '14px' }}>{c.consultation_date}</span>
                  </div>
                  <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
                    {anthro?.weight_kg && <span style={{ fontSize: '14px', color: 'var(--cyan)', fontFamily: 'JetBrains Mono' }}>{anthro.weight_kg} kg</span>}
                    {anthro?.body_fat_pct && <span style={{ fontSize: '14px', color: 'var(--amber)', fontFamily: 'JetBrains Mono' }}>{anthro.body_fat_pct}% GC</span>}
                    {anthro?.bmi && <span style={{ fontSize: '14px', color: 'var(--text-muted)', fontFamily: 'JetBrains Mono' }}>IMC: {Number(anthro.bmi).toFixed(1)}</span>}
                ...
                {isOpen && (
                  <div style={{ padding: '0 20px 20px', borderTop: '1px solid var(--border)' }}>
                    {anthro && (
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: '10px', paddingTop: '16px' }}>
                        {fields.map(f => {
                          const val = anthro[f.key]
                          if (!val) return null
                          return (
                            <div key={f.key} style={{ background: 'var(--bg-elevated)', borderRadius: '8px', padding: '10px' }}>
                              <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginBottom: '2px' }}>{f.label}</div>
                              <div style={{ fontWeight: 700, fontFamily: 'JetBrains Mono', fontSize: '14px' }}>{Number(val).toFixed(1)} {f.unit}</div>
                            </div>
                          )
                        })}
                      </div>
                    ...
                    {c.notes_specialist && (
                      <div style={{ marginTop: '16px', background: 'var(--cyan-dim)', borderRadius: '8px', padding: '12px', borderLeft: '3px solid var(--cyan)', fontSize: '14px', lineHeight: 1.6 }}>
                        <strong style={{ color: 'var(--cyan)' }}>Nota del especialista:</strong> {c.notes_specialist}
                      </div>
                    )}
                  </div>
                )}
